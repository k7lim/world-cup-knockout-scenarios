...
async function loadLocalData() {
  ...
  try {
    ...
    setOddsStatus(`Local tournament file loaded${source}. Use Update From GitHub Source for newer played results.`);
    renderAll();
  } catch (error) {
...
async function bootFromSources() {
  ...
  try {
    await seedFromOdds("syncOdds", { auto: true });
  } finally {
    setBusy(false);
  }
}
...
async function seedFromOdds(mode = "seedEmpty", options = {}) {
  ...
  const fixtures = eligibleOddsFixtures(mode);
  ...
  try {
    const payload = await postJson("/api/odds/predictions", {
      mode,
...
function clearPredictions() {
  ...
  savePredictions();
  saveMarketEvidence();
  setOddsStatus("Predictions cleared.");
  renderAll();
}
